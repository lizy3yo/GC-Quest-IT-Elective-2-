...
export default function QuizStudentSubmissionPage() {
  ...
  useEffect(() => {
    console.log('🎯 NEW ROUTE - Exam Student Submission Page loaded', { examId, studentId });
    if (examId && studentId) {
  ...
  const loadQuizAndSubmission = async () => {
    try {
      ...
      // Load exam details
      const quizRes = await fetch(`/api/teacher_page/assessment/${examId}`, {
      ...
      let submissionRes = await fetch(
        `/api/teacher_page/assessment/${examId}/student/${studentId}/submission`,
        {
          headers: { 'Authorization': `Bearer ${token}` }
        }
      );
      ...
      if (submissionRes.ok) {
        ...
        console.log('📊 Live Exam - Submission data received:', submissionData);
        if (submissionData.success) {
          let sub = submissionData.data.submission;
          console.log('🔍 Live Exam - Tab switching data from individual endpoint:', {
          ...
          if (sub.tabSwitches === undefined) {
            console.log('⚠️ Live Exam - Tab switching data missing, fetching from submissions list...');
            const submissionsRes = await fetch(
              `/api/teacher_page/assessment/${examId}/submissions`,
              {
                headers: { 'Authorization': `Bearer ${token}` }
              }
            );
            ...
            if (submissionsRes.ok) {
              ...
              console.log('📋 Live Exam - All submissions:', allSubmissions.map((s: any) => ({
                id: s._id,
              ...
              if (!matchingSubmission) {
                ...
                console.log('🔍 Live Exam - Using any submission from student:', matchingSubmission);
              }
              
              if (matchingSubmission) {
                console.log('✅ Live Exam - Found tab switching data:', {
                ...
                console.log('❌ Live Exam - No matching submission found for student:', studentId);
              }
            }
          } else {
            console.log('✅ Live Exam - Tab switching data already present:', {
              tabSwitches: sub.tabSwitches,
              tabSwitchDurations: sub.tabSwitchDurations,
              totalAwayMs: sub.totalAwayMs
            });
  ...
  const handleQuestionGrade = async (questionId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/teacher_page/assessment/${examId}/student/${studentId}/question-grade`,
        {
          method: 'PUT',
          headers: {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-rose-50 via-pink-50 to-purple-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-900">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-purple-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-900 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-xl mb-6 border border-slate-200 dark:border-slate-700">
          ...
          <button
            onClick={() => router.push(`/teacher_page/exam/${examId}`)}
