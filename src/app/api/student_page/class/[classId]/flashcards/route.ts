...
export async function GET(
  ...
  try {
    ...
    console.log('[Student Flashcards API] Fetching flashcards for class:', classId);

    // Get class details to find the teacher and subject
    const classDoc = await Class.findById(classId);
    if (!classDoc) {
      console.log('[Student Flashcards API] Class not found:', classId);
    ...
    console.log('[Student Flashcards API] Class found:', {
    ...
    console.log('[Student Flashcards API] Query:', query);
    
    const flashcards = await Flashcard.find(query).sort({ createdAt: -1 });
    
    console.log('[Student Flashcards API] Found flashcards:', flashcards.length);
