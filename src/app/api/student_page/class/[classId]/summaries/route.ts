...
export async function GET(
  ...
  try {
    ...
    console.log('[Student Summaries API] Fetching summaries for class:', classId);

    // Get class details to find the teacher and subject
    const classDoc = await Class.findById(classId);
    if (!classDoc) {
      console.log('[Student Summaries API] Class not found:', classId);
    ...
    console.log('[Student Summaries API] Class found:', {
    ...
    console.log('[Student Summaries API] Query:', query);
    
    const summaries = await Summary.find(query).sort({ createdAt: -1 });
    
    console.log('[Student Summaries API] Found summaries:', summaries.length);
