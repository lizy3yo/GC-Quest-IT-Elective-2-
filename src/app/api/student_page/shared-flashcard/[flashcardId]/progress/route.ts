...
export const GET = async (request: NextRequest, context: any) => {
  ...
  try {
    ...
    console.log("=== GET SHARED FLASHCARD PROGRESS RESPONSE ===");
    ...
    logger?.error?.("GET /shared-flashcard/progress error", { err });
...
export const PATCH = async (request: NextRequest, context: any) => {
  ...
  try {
    ...
    console.log("=== SHARED FLASHCARD API PATCH REQUEST ===");
    console.log("Received body:", JSON.stringify(body, null, 2));

    // Allowed top-level keys. Include learn/match/test and flashcards so namespace updates are permitted.
    const allowed = ["starredIds", "prefs", "sessionQueue", "viewerPos", "learn", "match", "test", "lastSessionStartedAt", "flashcards", "completion"];
    const setObj: any = {};
    ...
    for (const key of allowed) {
      ...
      if ((key === "learn" || key === "match" || key === "test" || key === "flashcards" || key === "completion") &&
    ...
    logger?.error?.("PATCH /shared-flashcard/progress error", { err });
