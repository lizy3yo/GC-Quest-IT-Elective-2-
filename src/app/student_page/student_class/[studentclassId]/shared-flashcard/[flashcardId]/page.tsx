...
export default function FlashcardOnlyPage() {
    ...
    async function load() {
      ...
      try {
        ...
        const response = await authManager.makeAuthenticatedRequest(
          `/api/student_page/shared-flashcard/${flashcardId}?userId=${finalUid}`,
  ...
  async function applyPersistedCardFavorites(loadedFlashcard: Flashcard, uidStr?: string) {
    try {
      ...
      if (uidToUse) {
        try {
          const pr = await fetch(`/api/student_page/shared-flashcard/${loadedFlashcard._id}/progress?userId=${uidToUse}`, { cache: 'no-store' });
          if (pr.ok) {
  ...
  async function saveProgress(payload: Record<string, unknown>) {
    ...
    try {
      const response = await fetch(`/api/student_page/shared-flashcard/${flashcardId}/progress?userId=${uid}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
  ...
  useEffect(() => {
    ...
    async function loadProgress() {
      ...
      try {
        const res = await fetch(`/api/student_page/shared-flashcard/${flashcardId}/progress?userId=${uid}`, { cache: "no-store" });
        if (!res.ok) {
  ...
  const gotoNextDeck = () => {
    // Navigate back to private library where user can pick next deck
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-3 sm:p-4 lg:p-6 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-2 sm:px-4">
        <header className="grid grid-cols-1 lg:grid-cols-[auto_1fr_auto] items-start lg:items-start gap-3 sm:gap-4 mb-4 sm:mb-6">
            ...
            <button
              onClick={() => {
                try {
                  ...
                  if (p.includes('/private_library/')) {
                    router.push('/student_page/private_library');
                    return;
                  }
                  if (p.includes('/public_library/')) {
            ...
            {/* Constrain shared flashcard title so long names don't push header controls */}
