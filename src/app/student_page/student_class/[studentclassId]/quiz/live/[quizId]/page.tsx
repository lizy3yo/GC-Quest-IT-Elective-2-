...
export default function StudentLiveQUIZPage({
  ...
  const [waitingForTeacher, setWaitingForTeacher] = useState(true); // Always start in waiting mode for live QUIZs
  ...
  useEffect(() => {
    if (!studentclassId || !quizId) {
      console.log('⏳ Waiting for params...', { studentclassId, quizId });
      return;
    }
    
    console.log('🚀 Starting fetch and join...', { studentclassId, quizId });
  ...
  const fetchAssessment = async () => {
    try {
      console.log('📥 Fetching assessment...', { studentclassId, quizId });
      ...
      showError("Failed to load quiz: " + errorMessage);
  ...
  const joinLiveSession = async () => {
    try {
      console.log('📞 Joining live session...', { studentclassId, quizId });
      const token = localStorage.getItem("accessToken");
      const response = await fetch(`/api/student_page/class/${studentclassId}/assessment/${quizId}/live-session`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` }
      });
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (isActive && !sessionStarted && !toastShownRef.current) {
          console.log('🎉 Teacher started the quiz!');
          toastShownRef.current = true;
          setSessionStarted(true);
          setWaitingForTeacher(false);
          showSuccess("Teacher has started the quiz!");
  ...
  const leaveLiveSession = async () => {
    try {
      ...
      const token = localStorage.getItem("accessToken");
      await fetch(`/api/student_page/class/${studentclassId}/assessment/${quizId}/live-session`, {
        method: "POST",
        headers: { 
  ...
  if (loading || !initialLoadComplete) {
    ...
    return <LoadingTemplate2 title="Loading live quiz..." />;
  }
  ...
  if (!assessment || !studentclassId || !quizId) {
    console.log('⏳ Waiting for data:', { assessment: !!assessment, studentclassId, quizId });
    return <LoadingTemplate2 title="Loading live quiz..." />;
  }

  // Show waiting room if session hasn't started
  // For live QUIZs, ALWAYS show waiting room unless session is explicitly started
  ...
  const handleQuestionAnswer = async (questionId: string, answer: any) => {
    try {
      const token = localStorage.getItem("accessToken");
      await fetch(`/api/student_page/class/${studentclassId}/assessment/${quizId}/live-answer`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
