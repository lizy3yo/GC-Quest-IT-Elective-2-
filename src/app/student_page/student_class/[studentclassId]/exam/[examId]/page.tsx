...
export default function StudentExamPage({
  ...
  useEffect(() => {
    if (!studentclassId || !examId) return;
    fetchAssessment();
  ...
  const fetchAssessment = async () => {
    try {
      ...
      // First fetch the assessment to check if it has a deadline (offline exam)
      ...
      // Check if this is an offline exam (has deadline) - no lobby needed
      const isOfflineExam = assessmentData.scheduledClose || assessmentData.dueDate;
      
      // Only check for live session if this is NOT an offline exam with deadline
      if (!isOfflineExam) {
        try {
          const liveCheckResponse = await fetch(`/api/student_page/class/${studentclassId}/assessment/${examId}/live-session`, {
            headers: { Authorization: `Bearer ${token}` }
          });
          ...
          if (liveCheckResponse.ok) {
            ...
            if (liveResult.data?.liveSession) {
              ...
              router.replace(`/student_page/student_class/${studentclassId}/exam/live/${examId}`);
              return;
            }
          }
        } catch (liveCheckError) {
          console.log('No live session detected, continuing with regular exam flow');
        }
        ...
        if (assessmentData.deliveryMode === 'live' || assessmentData.isLive || result.data.isLiveSession) {
          console.log('🔴 Live exam detected (deliveryMode: live), redirecting to live lobby...');
          router.replace(`/student_page/student_class/${studentclassId}/exam/live/${examId}`);
          return;
        }
      }
  ...
  if (loading) {
    return <LoadingTemplate2 title="Loading exam..." />;
  }
            ...
            <div className="relative">
              ...
              {accessMessage ? (
                ...
                <div className="bg-gradient-to-r from-red-50 to-rose-50 dark:from-red-900/20 dark:to-rose-900/20 rounded-xl p-4 mb-6 border border-red-200 dark:border-red-800">
                  ...
                  <button
                    onClick={() => router.push(`/student_page/student_class/${studentclassId}/assessment/${examId}/results`)}
  ...
  // Show the actual exam player after start button is clicked
  return (
