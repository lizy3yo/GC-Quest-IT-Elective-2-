...
export default function FlashcardOnlyPage() {
  ...
  }, [flashcardId, authLoading, isAuthenticated, user, applyPersistedCardFavorites]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300 overflow-x-hidden">
      <div className="w-full max-w-7xl mx-auto p-2 sm:p-3 lg:p-4">
        <header className="mb-3 sm:mb-4 w-full">
          {/* Title row - compact and separate from progress/settings */}
          <div className="flex items-center gap-2 mb-2">
            <button
              onClick={() => router.back()}
              aria-label="Back"
              className="w-8 h-8 rounded-full bg-slate-700/60 hover:bg-slate-700/80 flex items-center justify-center text-slate-100 transition flex-shrink-0"
            >
              <ChevronLeft size={14} className="text-slate-100" />
            </button>

            <div className="min-w-0 flex-1 overflow-hidden">
              <h1 className="text-xs sm:text-sm font-semibold text-slate-900 dark:text-slate-100 truncate leading-tight" title={flashcard.title}>{flashcard.title}</h1>
              <div className="text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 truncate">Flashcards · {flashcard.cards.length}</div>
            </div>
          </div>

          {/* Progress and Settings row - always visible, compact layout */}
          <div className="flex items-center gap-2 w-full">
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between text-[10px] sm:text-xs text-slate-500 dark:text-slate-400 gap-1">
                <div className="flex items-center whitespace-nowrap">
                  <span>Progress</span>
                  <span className="font-semibold text-slate-900 dark:text-slate-100 ml-1">{progress}%</span>
                </div>

                <div className="flex items-center gap-1 sm:gap-1.5">
                  <div className="inline-flex items-center gap-0.5 sm:gap-1 px-1 sm:px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-800 dark:bg-amber-900/20 whitespace-nowrap">
                    <span className="font-semibold text-[10px] sm:text-xs">Learning</span>
                    <span className="inline-block min-w-[16px] text-center bg-white dark:bg-slate-800 rounded-full px-0.5 text-[10px] sm:text-xs">{remaining}</span>
                  </div>
                  <div className="inline-flex items-center gap-0.5 sm:gap-1 px-1 sm:px-1.5 py-0.5 rounded-full bg-emerald-600/10 text-emerald-600 dark:bg-emerald-600/20 whitespace-nowrap">
                    <span className="font-semibold text-[10px] sm:text-xs">Know</span>
                    <span className="inline-block min-w-[16px] text-center bg-white dark:bg-slate-800 rounded-full px-0.5 text-[10px] sm:text-xs">{learnedCount}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              aria-label="Settings"
              title="Settings"
              className="flex-shrink-0 w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:shadow inline-flex items-center justify-center transition-shadow"
            >
              <Settings size={16} />
            </button>
          </div>
