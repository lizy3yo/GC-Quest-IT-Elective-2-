...
export default function PublicFlashcardOnlyPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        // Use public flashcard endpoint for public library
  ...
  // Read persisted favorite card timestamps (local only for public flashcards - no server progress)
  async function applyPersistedCardFavorites(loadedFlashcard: Flashcard, uidStr?: string) {
    try {
      const uidToUse = uidStr || uid;

      // For public flashcards, only use localStorage (no server progress)
      if (uidToUse) {
        try {
          ...
          if (raw) {
            ...
            const entries = Object.entries(map).sort((a, b) => b[1] - a[1]); // newest first
            const favIds = entries.map(e => e[0]);
            const sv = new Set<string>(favIds);
            setStarredIds(sv);
            
            // reorder cards so starred ones come first in timestamp order
            const remaining = loadedFlashcard.cards.filter(c => !sv.has(c._id));
            const starredCards = favIds.map((id: string) => loadedFlashcard.cards.find(c => c._id === id)).filter(Boolean) as FlashcardCard[];
  ...
  if (isLoading) return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300 flex items-center justify-center p-6">
      <div className="text-center">
        <div className="w-10 h-10 border-4 border-emerald-600 border-t-transparent rounded-full animate-spin mx-auto mb-4" aria-hidden="true" />
  ...
  // Dynamically constrain progress bar width based on title length
  const titleCharCount = flashcard.title.length;
  const progressWidthClass = titleCharCount < 28 ? 'max-w-[22rem]' : titleCharCount < 48 ? 'max-w-[42rem]' : 'max-w-[60ch]';
  ...
  const gotoNextDeck = () => {
    // Navigate back to public library where user can pick next deck
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-3 sm:p-4 lg:p-6 transition-colors duration-300">
      <div className="max-w-7xl mx-auto px-2 sm:px-4">
        <header className="grid grid-cols-1 lg:grid-cols-[auto_1fr_auto] items-start lg:items-start gap-3 sm:gap-4 mb-4 sm:mb-6">
            ...
            {/* Constrain long public flashcard titles to avoid pushing progress/settings off-screen */}
            <div className="min-w-0 flex-1 max-w-[60ch]">
              <h1
                className="text-base sm:text-lg lg:text-2xl font-semibold text-slate-900 dark:text-slate-100 truncate leading-tight"
          ...
          {/* Progress + stats + settings: top-aligned to avoid vertical shift when title is short */}
          <div className={`flex items-start gap-3 lg:justify-self-end px-1 sm:px-2 ${progressWidthClass}`}>
            <div className="flex-1">
              ...
              <div className="mt-1 flex flex-col sm:flex-row sm:items-start sm:justify-between text-xs sm:text-sm text-slate-500 dark:text-slate-400 gap-2 sm:gap-3">
                <div className="flex items-center">
                  <span>Progress</span>
                  <span className="font-semibold text-slate-900 dark:text-slate-100 ml-1 sm:ml-2">{progress}%</span>
                </div>
                <div className="flex items-center gap-2 sm:gap-3 flex-nowrap justify-start sm:justify-end overflow-x-auto">
                  <div className="inline-flex items-center gap-1 sm:gap-2 px-2 py-1 rounded-full bg-amber-100 text-amber-800 dark:bg-amber-900/20 text-xs sm:text-sm whitespace-nowrap">
                  ...
                  <div className="inline-flex items-center gap-1 sm:gap-2 px-2 py-1 rounded-full bg-emerald-600/10 text-emerald-600 dark:bg-emerald-600/20 text-xs sm:text-sm whitespace-nowrap">
            ...
            <button
              ...
              aria-label="Options"
              className="self-start sm:self-center px-2 sm:px-3 py-1 sm:py-1.5 rounded-full bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-200 hover:shadow inline-flex items-center text-sm sm:text-base"
            >
            ...
            <>
              ...
              <div className="mt-3 sm:mt-4 flex items-center justify-center gap-2 sm:gap-3 flex-wrap">
                ...
                <button
                  ...
                  disabled={isRatingInProgress}
                  className={`min-w-[60px] sm:min-w-[72px] px-2 sm:px-3 py-1.5 sm:py-2 rounded-full bg-emerald-600/10 text-emerald-600 text-sm sm:text-base font-medium hover:bg-emerald-600/20 transition-colors ${isRatingInProgress ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                  Good
                </button>
                <button
                  ...
                  disabled={isRatingInProgress}
                  className={`min-w-[60px] sm:min-w-[72px] px-2 sm:px-3 py-1.5 sm:py-2 rounded-full bg-emerald-600 text-white text-sm sm:text-base font-medium hover:bg-emerald-700 transition-colors ${isRatingInProgress ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                  Easy
                </button>
              </div>
              ...
              <div className="mt-4 sm:mt-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
                ...
                <div className="flex items-center gap-2 sm:gap-3 justify-center sm:justify-end">
                  ...
                  <button
                    ...
                    className="px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-full bg-gradient-to-br from-emerald-600 to-emerald-600 text-white shadow-sm hover:brightness-105 transition disabled:opacity-50 disabled:cursor-not-allowed"
                  >
            ...
            <div className="relative w-full max-w-4xl max-h-[95vh] bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-xl shadow-2xl overflow-auto grid grid-cols-1 lg:grid-cols-[1fr_220px] items-start">
              ...
              <div className="p-4 sm:p-6 h-full flex flex-col gap-3">
                <div className="flex items-start justify-between">
                  ...
                  <button onClick={() => setShowOptions(false)} className="ml-3 sm:ml-4 text-slate-400 hover:text-emerald-600 p-1 flex-shrink-0">
...
type SettingRowProps = { title: string; desc: string; checked: boolean; onToggle: () => void; accent?: "green" | "amber"; disabled?: boolean };
function SettingRow({ title, desc, checked, onToggle, accent = "green", disabled = false }: SettingRowProps) {
  const accentClass = accent === "amber" ? "from-amber-400 to-amber-500" : "from-green-400 to-emerald-500";
  const buttonBase = `w-12 h-6 rounded-full p-0.5 transition ${checked ? `bg-gradient-to-br ${accentClass}` : 'bg-slate-300 dark:bg-slate-700'}`;
  const buttonDisabled = disabled ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer';
    ...
    <section className="mt-8">
      <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-700 shadow p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
          <SummaryTile label="Topics mastered" value={mastered} className="bg-emerald-600/10 text-emerald-600 dark:bg-emerald-600/20" />
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <button onClick={onNextDeck} className="w-full px-4 py-3 rounded-md bg-emerald-600 text-white hover:brightness-110">Go to next deck</button>
