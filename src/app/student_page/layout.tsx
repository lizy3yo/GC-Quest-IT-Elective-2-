...
function StudentLayoutContent({ children }: StudentLayoutProps) {
  ...
  // Initialize as false to match server-side render, then update in useLayoutEffect
  ...
  useEffect(() => {
    ...
    const applyLocal = () => {
      try {
        ...
        else if (parsed.image) setUserImage(parsed.image);
      } catch {
        // ignore parse errors
      }
    };
    ...
    (async () => {
      try {
        ...
        if (typeof dbUser.profileImage === "string") setUserImage(dbUser.profileImage);
      } catch {
  ...
  // Check mobile/tablet on mount and window resize
  // Use useLayoutEffect to run synchronously before paint to prevent flash
  useLayoutEffect(() => {
    const checkMobile = () => {
      // Treat both mobile and tablet (< 1280px) as "mobile" for sidebar behavior
      // This includes tablets in the hamburger menu experience
      const mobile = window.innerWidth < 1280; // xl breakpoint - includes tablets
      setIsMobile(mobile);
      // Auto-expand sidebar on mobile/tablet
      if (mobile) {
        setIsSidebarCollapsed(false);
        // Close off-canvas if switching to mobile/tablet to avoid inconsistent transform states
  ...
  return (
    ...
    <div className={`min-h-screen bg-slate-50 transition-colors duration-300 ${resolvedTheme === 'dark' ? 'dark' : ''}`} style={resolvedTheme === 'dark' ? { backgroundColor: '#090909' } : {}}>
      {/* Mobile Top Navbar */}
      {isMobile && (
        <header className={`fixed top-0 left-0 right-0 h-14 z-[70] border-b border-slate-200 dark:border-slate-800 px-3 sm:px-6 backdrop-blur ${
          ...
          <div className="flex items-center justify-between h-full">
            ...
            <button
              ...
              className="p-2 rounded-lg text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 focus:outline-none focus:ring-2 focus:ring-[#1C2B1C] focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-slate-900 flex-shrink-0"
            >
              {/* Hamburger icon */}
            ...
            <div className="flex-1 flex justify-center">
              <Link href="/student_page/dashboard" className="flex items-center no-underline">
                <span className="text-lg font-bold tracking-tight whitespace-nowrap">
                  <span className="inline dark:hidden bg-gradient-to-br from-green-500 to-green-600 bg-clip-text text-transparent">GCQuest</span>
                  <span className="hidden dark:inline text-green-400">GCQuest</span>
                </span>
              </Link>
            </div>
      ...
      <div className="flex">
                    ...
                    <div className="relative">
                      {!isExpanded ? (
                        ...
                        <button
                          ...
                          className="flex items-center gap-4 px-5 py-2.5 text-slate-500 dark:text-slate-400 text-sm font-medium transition-all duration-300 mx-4 w-[calc(100%-2rem)] rounded-xl hover:bg-[#E8F5E9] dark:hover:bg-slate-800 hover:text-[#2E7D32] dark:hover:text-slate-200 hover:translate-x-1 bg-transparent border-none cursor-pointer"
                        >
                          <svg
                            className="flex-shrink-0 transition-transform duration-300 hover:scale-110"
                            width="20"
                  ...
                  <div className="mb-4">
                    ...
                    <Link
                      ...
                      className={`flex items-center ${
                        ...
                        pathname.startsWith("/student_page/study_rooms")
                          ? "bg-[#E8F5E9] text-[#2E7D32] font-semibold shadow-[0_2px_8px_rgba(46,125,50,0.15)]"
                          : ""
                      }`}
                      title={!isExpanded ? "Study Rooms" : ""}
                    >
                      <svg
                        className={`flex-shrink-0 transition-transform duration-300 hover:scale-110 ${
                          pathname.startsWith("/student_page/study_rooms")
                      ...
                      {isExpanded && (
                        <span className="whitespace-nowrap">Study Rooms</span>
                      )}
                    </Link>

                    <Link
                      href="/student_page/leaderboards"
                      className={`flex items-center ${
                          !isExpanded
                            ? "justify-center w-full p-2.5 mx-0"
                            : "gap-4 px-5 py-2.5 mx-4"
                        ...
                        pathname === "/student_page/leaderboards"
                          ? "bg-[#E8F5E9] text-[#2E7D32] font-semibold shadow-[0_2px_8px_rgba(46,125,50,0.15)]"
                          : ""
                      }`}
                      title={!isExpanded ? "Leaderboards" : ""}
                    >
                      <svg
                        className={`flex-shrink-0 transition-transform duration-300 hover:scale-110 ${
                          pathname === "/student_page/leaderboards"
                      ...
                      {isExpanded && (
                        <span className="whitespace-nowrap">Leaderboards</span>
                      )}
                    </Link>

                    <Link
                      href="/student_page/resources"
                      className={`flex items-center ${
                          !isExpanded
                            ? "justify-center w-full p-2.5 mx-0"
                            : "gap-4 px-5 py-2.5 mx-4"
                        ...
                        pathname === "/student_page/resources"
                          ? "bg-[#E8F5E9] text-[#2E7D32] font-semibold shadow-[0_2px_8px_rgba(46,125,50,0.15)]"
                          : ""
                      }`}
        ...
        >
          ...
          <div className="sticky top-0 z-40 bg-slate-50 dark:bg-[#090909] px-8 py-6">
            <div className="flex items-center justify-between gap-6">
              ...
              {!isMobile && isMounted && currentTime && (
                <div className="text-sm font-medium text-slate-600 dark:text-slate-400 whitespace-nowrap flex-shrink-0">
                  {currentTime}
                </div>
              )}
            </div>
      ...
      {/* Chatbot for authenticated students */}
